...
import { $parentDoc } from '../../store/current'
import {
  editDocumentStores,
  editDocumentEvents,
  $editDocument,
...
function ParentDocEditNavigation() {
  const history = useHistory()
  const goBack = useCallback(() => history.goBack(), [history])
...
function ParentDocEdit() {
  return (
    <SidebarPanel
      header={'Edition | Document | Assiette'}
      navigation={<ParentDocEditNavigation />}
      main={<ParentDocEditMain />}
      action={<ParentDocEditAction />}
    />
  )
}

export default ParentDocEdit
